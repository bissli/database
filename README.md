...
Execute stored procedures or SQL statements that may return multiple result sets:

```python
# Basic stored procedure call
result = db.callproc(cn, 'CALL get_users_by_status(%s)', 'active')

# With multiple statements that return different result sets
result_sets = db.callproc(cn, """
    SELECT id, name FROM users WHERE active = true;
    SELECT COUNT(*) AS total_count FROM users;
""", return_all=True)

# Return the largest result set (default behavior)
largest_result = db.callproc(cn, multi_statement_sql)

# Return the first result set regardless of size
first_result = db.callproc(cn, multi_statement_sql, prefer_first=True)

# Return all result sets as a list
all_results = db.callproc(cn, multi_statement_sql, return_all=True)
```

The `callproc` function is particularly useful for:
- Stored procedures that return multiple result sets
- SQL Server procedures where NOCOUNT is OFF
...
The database module automatically handles common Python, NumPy, and Pandas data types:
...
The module automatically converts various Python data types when writing to the database:

- Python native types (`str`, `int`, `float`, `bool`, etc.) are converted to appropriate database types
- NumPy, Pandas, and PyArrow types are converted to compatible database types
- Special values like `None`, `NaN`, and `pd.NA` are properly converted to SQL NULL values

Type conversion happens transparently - you don't need to manually format data before using it in queries.

If incompatible types are provided, a `TypeConversionError` may be raised. This
can happen when trying to convert complex objects or types that don't have an
appropriate database representation.

### Database-Specific Type Handling

- **PostgreSQL**: Custom adapters for numeric types (NaN handling) and array types
- **SQLite**: Date/time handling through ISO format conversion
- **SQL Server**: Type mapping from database-specific types to Python types
...
cn = db.connect({
    ...
    'database': 'your_database',
    # other connection parameters...
}, use_pool=True, pool_max_connections=10, pool_max_idle_time=300)
...
- `select(cn, sql, *args, **kwargs)` - Execute SELECT query, return DataFrame
...
- `callproc(cn, sql, *args, **kwargs)` - Execute stored procedures that may return multiple result sets
