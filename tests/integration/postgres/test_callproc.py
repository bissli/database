...
def test_callproc_basic(psql_docker, conn):
    """Test basic callproc functionality with a simple query"""
    # Create a function that returns a result set
    # First check the type of the name column
    column_type = db.select_scalar(conn, """
    ...
    # Call using callproc
    result = db.callproc(conn, 'SELECT * FROM get_test_data()')
...
def test_callproc_multiple_resultsets(psql_docker, conn):
    """Test callproc with a function returning multiple result sets"""
    ...
    all_results = db.callproc(conn, sql, return_all=True)
    ...
    assert all_results[1].iloc[0]['name'] == 'Summary'

    # Default behavior should return largest result set
    default_result = db.callproc(conn, sql)
    assert len(default_result) == len(all_results[0])

    # prefer_first should return first result set
    first_result = db.callproc(conn, sql, prefer_first=True)
    assert len(first_result) == len(all_results[0])
    pd.testing.assert_frame_equal(first_result, all_results[0])


def test_callproc_empty_resultset(psql_docker, conn):
    """Test callproc with empty result sets"""
    ...
    result = db.callproc(conn, sql)
    assert isinstance(result, pd.DataFrame)
    assert result.empty

    # With return_all, should return list of empty DataFrames
    all_results = db.callproc(conn, sql, return_all=True)
    assert isinstance(all_results, list)
    assert len(all_results) == 2
    assert all_results[0].empty
    assert all_results[1].empty
