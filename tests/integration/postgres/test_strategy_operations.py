...
def test_reindex_table(psql_docker, conn):
    ...
    # Perform reindex
    db.reindex_table(conn, 'test_table')
    
    # Verify the index still exists
    index_exists = db.select_scalar(conn, """
        SELECT EXISTS (
...
def test_cluster_table(psql_docker, conn):
    ...
    # Perform cluster
    db.cluster_table(conn, 'test_table', 'test_idx_cluster')
...
def test_strategy_get_primary_keys(psql_docker, conn):
    ...
    pk_columns = strategy.get_primary_keys(conn, 'test_composite_pk')
    
    # Verify primary keys
    assert set(pk_columns) == {'id1', 'id2'}
    
    # Test with the regular table
    pk_columns = strategy.get_primary_keys(conn, 'test_table')
    assert pk_columns == ['name'], "Should correctly identify 'name' as primary key"


def test_strategy_get_sequence_columns(psql_docker, conn):
    ...
    strategy = PostgresStrategy()
    seq_columns = strategy.get_sequence_columns(conn, 'test_sequence_columns')
